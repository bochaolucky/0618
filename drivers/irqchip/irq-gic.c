...
{
	pr_info("LHC|gic_of_init begin...\r\n");
	struct gic_chip_data *gic;
	int irq, ret;

	if (WARN_ON(!node))
	{
		pr_info("LHC|gic_of_init mark 1...\r\n");
		return -ENODEV;
	}

	if (WARN_ON(gic_cnt >= CONFIG_ARM_GIC_MAX_NR))
	{
		pr_info("LHC|gic_of_init mark 2...\r\n");
		return -EINVAL;
	}
	...
	if (ret)
	{
		pr_info("LHC|gic_of_init mark 3...\r\n");
		return ret;
	}
	...
	{
		pr_info("LHC|gic_of_init mark 4...\r\n");
		static_key_slow_dec(&supports_deactivate);
	}
	...
	if (ret) {
		pr_info("LHC|gic_of_init mark 5...\r\n");
		gic_teardown(gic);
		return ret;
	}

	if (!gic_cnt) {
		pr_info("LHC|gic_of_init mark 6...\r\n");
		gic_init_physaddr(node);
		gic_of_setup_kvm_info(node);
	}

	if (parent) {
		pr_info("LHC|gic_of_init mark 7...\r\n");
	...
	{
		pr_info("LHC|gic_of_init mark 8...\r\n");
		gicv2m_init(&node->fwnode, gic_data[gic_cnt].domain);
	}

	gic_cnt++;
	pr_info("LHC|gic_of_init end\r\n");
	return 0;
}
