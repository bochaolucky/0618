...
#define APB_UART1_BASE      0xf8000100
//#define APB_UART1_BASE      0xc4804100  //@srliu@0530
...
// {
//     char buffer[1024]; // 格式化字符串的缓冲区，需要时调整大小

//     /* 格式化字符串 */
//     vsnprintf(buffer, sizeof(buffer), fmt, args);

//     /*
//      * 即使在非掩蔽中断（NMI）中，也尝试使用主日志缓冲区。但避免调用可能有自己锁的控制台驱动。
//      */
//     if ((this_cpu_read(printk_context) & PRINTK_NMI_DIRECT_CONTEXT_MASK) &&
//         raw_spin_trylock(&logbuf_lock)) {
//         putstr(buffer); // 使用 putstr 输出格式化后的字符串
//         raw_spin_unlock(&logbuf_lock);
//         defer_console_output();
//         return strlen(buffer); // 返回输出字符串的长度
//     }

//     /* 当 logbuf_lock 被占用或在安全模式下，在 NMI 中使用额外的缓冲区。*/
//     if (this_cpu_read(printk_context) & PRINTK_NMI_CONTEXT_MASK)
//         return vprintk_nmi(fmt, args); // 假设 vprintk_nmi 处理输出

//     /* 在安全模式下，使用额外的缓冲区以防递归死锁。*/
//     if (this_cpu_read(printk_context) & PRINTK_SAFE_CONTEXT_MASK)
//         return vprintk_safe(fmt, args); // 假设 vprintk_safe 处理输出
